  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
      <div className="space-y-6">
        <h3 className="text-xl md:text-2xl font-black text-gray-900">Dados do Vínculo</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div><label className="text-[10px] font-black uppercase text-gray-400">Admissão</label><input type="date" value={admissionDate} onChange={(e) => setAdmissionDate(e.target.value)} className="w-full p-3 rounded-xl bg-gray-50 ring-1 ring-gray-200 outline-none font-bold mt-1 text-sm" /></div>
          <div><label className="text-[10px] font-black uppercase text-gray-400">Demissão</label><input type="date" value={dismissalDate} onChange={(e) => setDismissalDate(e.target.value)} className="w-full p-3 rounded-xl bg-gray-50 ring-1 ring-gray-200 outline-none font-bold mt-1 text-sm" /></div>
        </div>
        <div>
          <label className="text-[10px] font-black uppercase text-gray-400">Último Salário Bruto</label>
          <input type="text" value={salary} onChange={(e) => handleCurrencyInput(e.target.value, setSalary)} className="w-full p-4 rounded-xl bg-gray-50 ring-1 ring-gray-200 outline-none text-lg md:text-xl font-black mt-1" placeholder="R$ 0,00" />
        </div>
        <select value={terminationType} onChange={(e:any) => setTerminationType(e.target.value)} className="w-full p-4 rounded-xl bg-gray-50 ring-1 ring-gray-200 outline-none font-bold appearance-none text-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <select value={noticeType} onChange={(e:any) => setNoticeType(e.target.value)} className="w-full p-3 rounded-xl bg-gray-50 ring-1 ring-gray-200 outline-none font-bold text-sm">
          ...
          <button onClick={() => setHasVestedVacation(!hasVestedVacation)} className={`p-3 rounded-xl text-xs font-bold border-2 transition-all ${hasVestedVacation ? 'border-red-600 bg-red-50 text-red-600' : 'border-gray-100 text-gray-400'}`}>Férias Vencidas: {hasVestedVacation ? 'Sim' : 'Não'}</button>
        </div>
      </div>
      <div className="bg-red-900 rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 text-white flex flex-col justify-center min-h-[300px] lg:min-h-[400px] shadow-2xl shadow-red-100 overflow-hidden">
        {results ? (
        ...
        ) : <div className="text-center opacity-20 py-10"><i className="fas fa-file-signature text-5xl"></i><p className="mt-4 text-[10px] font-bold uppercase tracking-widest">Aguardando dados</p></div>}
      </div>
    </div>
  );
};
