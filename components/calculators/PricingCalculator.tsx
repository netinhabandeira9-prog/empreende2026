...
const PricingCalculator: React.FC = () => {
  ...
  const formatBRL = (val: number) => {
    // \u00A0 é um espaço não-quebrável que impede o R$ de separar do número
  ...
  // Escala dinâmica para que o número nunca quebre ou suma
  const getDynamicFontSize = (text: string, type: 'hero' | 'grid') => {
    ...
    if (type === 'grid') {
      if (len > 15) return 'text-[9px] sm:text-xs';
  ...
  const results = useMemo<CalculationResult | null>(() => {
    ...
    // Forçamos a quantidade a ser pelo menos 1 para o cálculo, mesmo que o input esteja vazio visualmente
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-10 items-start">
      ...
      <div className="space-y-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Configurações de Venda</p>
          </div>
        </div>
        ...
        <div className="space-y-5">
          <div>
            <label className="block text-[10px] font-black uppercase text-gray-400 mb-1 ml-1">Custo Total (R$)</label>
          ...
          <button onClick={() => setIncludeTax(!includeTax)} className={`w-full p-4 rounded-2xl border-2 flex items-center justify-between transition-all ${includeTax ? 'border-purple-600 bg-purple-50 text-purple-700 shadow-sm' : 'border-gray-100 text-gray-400'}`}>
            <div className="text-left">
              <p className="text-[10px] font-black uppercase">Simular Impostos (1%)</p>
              <p className="text-[9px] font-bold opacity-60 italic">Alíquota de teste 2026</p>
            </div>
      ...
      {/* Resultados - Design Corrigido e Fluído */}
      <div className="bg-purple-900 rounded-[3rem] p-8 md:p-12 text-white flex flex-col justify-center shadow-2xl min-h-[500px] overflow-hidden relative border-8 border-purple-800/20">
        {results && !results.error ? (
          <div className="animate-fadeIn w-full flex flex-col items-center space-y-10">
            <div className="text-center w-full">
              <p className="text-[10px] font-black text-purple-300 uppercase tracking-[0.3em] opacity-80 mb-2">
                {calcMode === 'BATCH' ? 'Preço do Lote Sugerido' : 'Preço de Venda Sugerido'}
              </p>
              ...
              {calcMode === 'BATCH' && (
                 <div className="inline-block bg-white/10 px-4 py-1.5 rounded-full mt-2">
                    <p className="text-[11px] font-bold text-purple-200">Unitário: {formatBRL(results.unitSalePrice)}</p>
                 </div>
              )}
            </div>

            <div className="grid grid-cols-2 gap-4 w-full">
              <div className="bg-white/5 rounded-[2rem] p-6 text-center border border-white/10 flex flex-col justify-center min-h-[100px]">
                <p className="text-[9px] font-black text-green-400 uppercase mb-2 tracking-widest">Lucro {calcMode === 'BATCH' ? 'Total' : 'Líquido'}</p>
          ...
          <div className="text-center p-10 bg-red-500/10 rounded-[2.5rem] border-2 border-red-500/30">
            ...
            <p className="text-xs font-black text-red-100 uppercase tracking-widest">{results.error}</p>
          </div>
        ) : (
          <div className="text-center opacity-30 py-10 space-y-6 flex flex-col items-center">
            ...
            <p className="text-[11px] font-black uppercase tracking-[0.3em] leading-relaxed">Insira os Valores<br/><span className="text-[9px] font-bold opacity-60 tracking-normal italic">Cálculo em Tempo Real</span></p>
          </div>
        )}
      </div>
    </div>
