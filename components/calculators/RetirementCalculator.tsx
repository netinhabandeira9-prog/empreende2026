...
const RetirementCalculator: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
      ...
      <div className="bg-amber-900 rounded-[2.5rem] p-10 text-white flex flex-col justify-center text-center shadow-2xl shadow-amber-100 min-h-[400px]">
        {results ? (
          <div className="space-y-8">
            <div className="bg-white/10 p-10 rounded-[2.5rem] border border-white/5">
              <p className="text-[10px] font-black text-amber-300 uppercase mb-2">Pontuação Atual</p>
              <h4 className="text-7xl font-black">{results.points}</h4>
              <p className="text-xs font-bold text-amber-200 mt-2 uppercase tracking-widest">Meta 2026: {results.target} Pontos</p>
            </div>
            {results.missing <= 0 ? (
              <div className="bg-green-500 p-4 rounded-2xl text-xs font-black uppercase">✅ Elegível para Requerer</div>
            ) : (
              <p className="text-sm italic text-amber-100">Faltam aproximadamente {results.missing} pontos para atingir a meta.</p>
            )}
          </div>
        ) : <div className="opacity-20"><i className="fas fa-hourglass-half text-6xl"></i><p className="mt-4 text-xs font-bold uppercase tracking-widest">Verificar Vínculos</p></div>}
      </div>
    </div>
  );
};
